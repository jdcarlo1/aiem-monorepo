...
- `verification_status` has two honest tiers for inline functions:
  ...
  - `VERIFIED_EXISTS` — function confirmed real (non-stub) and its own direct effect confirmed, but its *own* deeper upstream chain belongs to a phase not yet reached (e.g. `_run_five_layer_conviction`'s L1-L8 signal inputs are Phase 9 work) — note this explicitly in `upstream_dependencies` rather than faking the trace.
