...
- **UNUSUAL CALLS cards show a "first detected" DAY badge, never a precise time.** Each `/stock-api/unusual-calls` hit carries `detected_label` (`Today`/`Yesterday`/`%b %-d`, computed ET via `_detected_label`) + `first_seen` (iso) from the DB `first_seen` column (preserved across upserts). **Why date-only:** `first_seen` is *scan-detection* time from scheduled scans (EOD scans stamp ~midnight/EOD), NOT the real order-placement time — a precise "9:47 AM" would mislead the owner. Do NOT regress this into a timestamp. Live-scan path is enriched by `_save_unusual_calls_to_db` via `RETURNING first_seen` (in-place dict mutation); save MUST run before `out` is built/cached or a concurrent request can read date-less cards.
